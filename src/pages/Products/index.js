        ...
        <>
            ...
            <Grid container sx={{
                ...
                <Grid sx={{ display: 'flex', justifyContent: 'center' }}>
                    <Box maxWidth={'md'} sx={{ flexGrow: 1, p: 5, backgroundColor: '#fff', borderRadius: '30px', }} >
                        <Grid
                            container
                            spacing={2}
                            sx={(theme) => ({
                        ...
                        >
                            {filteredProducts.map((product, index) => (
                                <Grid key={index} item {...colWidth} minHeight={160} sx={{ display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center' }}>
                                    <Box component={'img'} src={product.img} sx={{ width: '223px', height: 'auto' }} />
                                    <Grid container justifyContent={'center'} mt={2}>
                                        <Typography sx={{ color: '#3D3F61', fontWeight: 800 }}>{product.name}</Typography>
                                        <Grid container spacing={1} justifyContent={'center'}>
                                            <Grid item xs={6}>
                                                <Typography sx={{ display: 'flex', color: '#727490', fontWeight: 200 }}>
                                                    {product.category}
                                                    <SvgIcon>
